-- docs/supabase/daily_batch_2026-05-22.sql
--
...
--   1. Upserts today's channel-manager reservations (the 5-22 export, 22 rows).
--      Changed reservations (e.g. Karen Smith's shifted dates) update in place.
...
--      booking that disappeared. (Bryan: "keep guest info, do not push to cards
--      ... unless it's the same guest and should be pushed" — same-guest rows
--      that are still in the file keep their external_id and stay active.)
...
--      OPEN maintenance issue (so Angie's reports never vanish) and leaves all
--      manually-added admin/manager cards (source='manual') untouched.
...
-- =========================================================================
-- 1. Upsert today's reservations (5-22 channel-manager export — 22 rows).
...
from (values
  ...
  ('rx-67655-31', 'Karen Smith',           '31', date '2026-05-21', date '2026-05-22', time '17:00', null::text,                          null::text[],          '{"res":"67655","channel":"Online","guest_raw":"Karen Smith","rooms":"31 Queen: 5/21/2026","total":"$138.13"}'::jsonb),
...
where external_id like 'rx-%'
  ...
  and external_id not in (
    'rx-67306-39','rx-67305-33','rx-67290-21','rx-67291-23','rx-67292-25',
    'rx-67307-27','rx-67600-37','rx-67158-29','rx-67655-31','rx-67342-26',
    'rx-67343-22','rx-67669-36','rx-66972-27','rx-67610-37','rx-67172-28',
    'rx-67609-39','rx-67308-38','rx-67670-42','rx-67599-33','rx-66850-41',
    'rx-66851-36','rx-67602-32'
  );
...
-- =========================================================================
-- Verification (run after applying — expect 8 / 2 / 1 on 2026-05-22):
-- =========================================================================
-- select context->>'staff_home_bucket' as bucket, count(*)
--   from public.tasks where source='pms' group by 1 order by 1;
--   -- expect: arrivals 2, departures 8, dailys 1, eod 1, stayovers 1, start_of_day 1
