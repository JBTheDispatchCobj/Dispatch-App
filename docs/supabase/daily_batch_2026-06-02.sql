-- docs/supabase/daily_batch_2026-06-02.sql
--
...
--   1. Upserts today's channel-manager reservations (the 6-2 export, 30 Block A
--      rows → 31 reservation rows; Mary Charmoli #67345 splits into rooms 21+23).
--      Changed reservations update in place (e.g. Tamzid #67309 shortened his
--      stay, so his dates/total correct themselves via on conflict).
...
--      booking that disappeared.
--   3. Rebuilds the channel-manager cards for TODAY from current_date, so
--      stayovers that checked out flip to departures, the night counter
--      advances, extensions re-bucket, etc. SPARES any card that still has an
--      OPEN maintenance issue and leaves manual (source='manual') cards alone.
...
--
-- NOTE ON CATCH-UP DAYS: the last batch run was 5-26; 5-27 through 6-1 were
-- never run. That is fine — the SQL is date-relative to current_date and this
-- 6-2 export is the current photo of the books, so running this one today
-- reconciles everything. Older batches in the series are file-only.
--
...
-- =========================================================================
-- 1. Upsert today's reservations (6-2 channel-manager export — 31 rows).
...
select
  ...
  v.raw_payload
from (values
  ('rx-67309-41', 'Tamzid Bin Mafiz',     '41', date '2026-05-31', date '2026-06-03', null::time,   null::text,                                                    array['$55 a night'],   '{"res":"67309","channel":"Phone","guest_raw":"Tamzid Bin Mafiz, *** ($55 a night)","rooms":"41 Queen: 5/31/2026-6/2/2026","total":"$182.34","rate_note":"$55 a night"}'::jsonb),
  ...
  ('rx-67890-31', 'Megan Kuhl',            '31', date '2026-06-01', date '2026-06-02', null::time,   null::text,                                                    null::text[],           '{"res":"67890","channel":"Phone","guest_raw":"Megan Kuhl","rooms":"31 Queen: 6/1/2026","total":"$149.18"}'::jsonb),
  ...
  ('rx-67848-33', 'DENNIS EDWARDS',        '33', date '2026-06-01', date '2026-06-02', null::time,   null::text,                                                    null::text[],           '{"res":"67848","channel":"Direct Connect","guest_raw":"DENNIS EDWARDS","rooms":"33 Queen: 6/1/2026","total":"$158.85"}'::jsonb),
  ('rx-67831-42', 'Angie Aviles',          '42', date '2026-06-01', date '2026-06-05', null::time,   null::text,                                                    null::text[],           '{"res":"67831","channel":"Phone","guest_raw":"Angie Aviles, ***","rooms":"42 ADA King Jacuzzi Room: 6/1/2026-6/4/2026","total":"$552.52"}'::jsonb),
  ('rx-67882-28', 'Travis Campton',        '28', date '2026-06-01', date '2026-06-02', null::time,   null::text,                                                    null::text[],           '{"res":"67882","channel":"Phone","guest_raw":"Travis Campton, ***","rooms":"28 Double : 6/1/2026","total":"$71.83"}'::jsonb),
  ...
  ('rx-67791-27', 'Bill Woock',            '27', date '2026-06-01', date '2026-06-02', null::time,   null::text,                                                    null::text[],           '{"res":"67791","channel":"Phone","guest_raw":"Bill Woock","rooms":"27 Queen: 6/1/2026","total":"$117.41"}'::jsonb),
  ('rx-67893-22', 'Joel Royer',            '22', date '2026-06-01', date '2026-06-02', null::time,   null::text,                                                    null::text[],           '{"res":"67893","channel":"Phone","guest_raw":"Joel Royer","rooms":"22 Double: 6/1/2026","total":"$110.50"}'::jsonb),
  ...
  ('rx-67894-26', 'Devvie Cersine',        '26', date '2026-06-01', date '2026-06-02', null::time,   null::text,                                                    null::text[],           '{"res":"67894","channel":"Phone","guest_raw":"Devvie Cersine","rooms":"26 ADA Double Room: 6/1/2026","total":"$124.31"}'::jsonb),
  ...
  ('rx-67790-28', 'Thomas Updyke',         '28', date '2026-06-02', date '2026-06-04', null::time,   'May have dog, double check. Told them fee would be $15/day if they do. Referred them to https://www.blueruff.com/ for pet boarding otherwise. It is him and his mom.', null::text[], '{"res":"67790","channel":"Phone","guest_raw":"Thomas Updyke","rooms":"28 Double : 6/2/2026-6/3/2026","total":"$248.62","note":"May have dog, double check. Told them fee would be $15/day if they do. Referred them to https://www.blueruff.com/ for pet boarding otherwise. It is him and his mom."}'::jsonb),
  ('rx-67869-33', 'Steven Ackerman',       '33', date '2026-06-02', date '2026-06-03', time '15:30', null::text,                                                    null::text[],           '{"res":"67869","channel":"Direct Connect","guest_raw":"Steven Ackerman","rooms":"33 Queen: 6/2/2026","total":"$138.13"}'::jsonb),
  ('rx-67776-27', 'Greg Jaroch',           '27', date '2026-06-02', date '2026-06-03', time '17:00', null::text,                                                    null::text[],           '{"res":"67776","channel":"Online","guest_raw":"Greg Jaroch","rooms":"27 Queen: 6/2/2026","total":"$138.13"}'::jsonb),
  ('rx-67895-31', 'Jim & Julie Brodtmann', '31', date '2026-06-03', date '2026-06-04', null::time,   'WIFES NAME IS JUILE',                                         array['$80 rate'],      '{"res":"67895","channel":"Phone","guest_raw":"Jim & Julie Brodtmann ($80 rate)","rooms":"31 Queen: 6/3/2026","total":"$88.40","rate_note":"$80 rate","preferences":"WIFES NAME IS JUILE"}'::jsonb),
  ...
  ('rx-66943-39', 'Tamzid Bin Mafiz',     '39', date '2026-06-07', date '2026-06-11', null::time,   null::text,                                                    array['$55 a night'],   '{"res":"66943","channel":"Phone","guest_raw":"Tamzid Bin Mafiz, *** ($55 a night)","rooms":"39 Queen: 6/7/2026-6/10/2026","total":"$337.03","rate_note":"$55 a night"}'::jsonb),
...
where external_id like 'rx-%'
  ...
  and external_id not in (
    'rx-67309-41','rx-67836-23','rx-67828-39','rx-66853-38','rx-67890-31',
    'rx-67833-37','rx-67832-35','rx-67848-33','rx-67831-42','rx-67882-28',
    'rx-67345-21','rx-67345-23','rx-67861-25','rx-67791-27','rx-67893-22',
    'rx-67883-24','rx-67894-26','rx-67329-29','rx-67790-28','rx-67869-33',
    'rx-67776-27','rx-67895-31','rx-67774-27','rx-67524-32','rx-65583-41',
    'rx-66825-24','rx-67757-23','rx-67755-42','rx-66943-39','rx-67482-41',
    'rx-67681-33'
  );
...
-- =========================================================================
-- Verification (run after applying — expect departures 6 / arrivals 3 /
-- stayovers 11 on 2026-06-02):
-- =========================================================================
-- select context->>'staff_home_bucket' as bucket, count(*)
--   from public.tasks where source='pms' group by 1 order by 1;
--   -- expect: arrivals 3, departures 6, dailys 1, eod 1, stayovers 11, start_of_day 1
