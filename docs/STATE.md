# Dispatch — Product State (current as of Day 38, 2026-05-07)
...
**Section II:** II.A (`AddTaskModal`) is ~95% built per Day 28 audit; confirmation pass pending (now chase #5 below). II.B / II.E / II.F / II.G non-Reassign surface CLOSED Day 36. II.H Admin Maintenance still PARTIAL (chase #3 below).
...
1. **III.B Phase 5 — `maintenance_issues` source swap in activity feed** (~30 min). Replace placeholder `notes WHERE note_type='Maintenance'` source in `lib/activity-feed.ts:12` with a real `maintenance_issues` query branch. Day 34's `reassigned`-as-warn classification + the existing severity-boost ordering means High-severity maintenance items will surface above info entries via the same path once the source swap lands.

2. **III.E + V.G photo pipeline wiring** (1-2 hours). `uploadTaskFile` helper exists at `lib/task-events.ts:45`. Wire into `NoteComposeForm` + `MaintenanceComposeForm`: file input + call `uploadTaskFile` + pass `imageUrl` to `addNote` / `addMaintenanceIssue`. Storage RLS policies need verification (master plan VII.F PARTIAL).

3. **II.H Admin Maintenance live-data wiring** (~1-2 hours). Schema unblocked by Day 33's `maintenance_issues` table. Wire `app/admin/maintenance/[id]/page.tsx` to query `public.maintenance_issues` filtered by location / by type, plus per-issue card view. Replace `ORDER` mock const.

4. **IV.H Wed-occupancy Deep Clean trigger** (~1 hour). Constants exist in `dispatch-config.ts` Section 12. Unblocked by III.D's audit-event sink (Day 29). All four conditions: <5 departures + 40%+ occupancy in last 45 days + no deep clean in 45 days + ≤3 deep items completed in 45 days. Auto-elevates Standard → Deep on Wednesdays.

5. **II.A `AddTaskModal` confirmation pass** (~30 min). Verify the modal matches master plan II.A spec end-to-end. Possible bucket-model tweak — maintenance currently routes to `staff_home_bucket: "start_of_day"` since maintenance has no staff bucket; confirm or surface as own bucket.

6. **I.G remaining sub-items.** Last Stayover Status lookup (was blocking on V.A BR4 — unblocked Day 37; sources from `tasks` history, not `reservations`), checklist variants for Sheet Change weekly + * guest (pending Jennifer's KB authoring), status-driven auto-complete (DND/Desk OK/Guest OK pre-selection auto-completes + auto-archives), Sheet Change skip semantics. Bundled or split as makes sense.

7. **Item I — Vercel deploy** (~30 min). Bryan's parallel lane via `docs/deployment/vercel-checklist.md`. GitHub push → Vercel CLI → first deploy → env vars (incl. `AGENT_KILL=true` and `AGENT_DRY_RUN=true` for safety) → Supabase magic-link redirect URL config → smoke test in incognito.

8. **V.C Cloudbeds.** Bryan's separate thread, outside engineering critical path. Pending sales quote.
...
The master plan footer (Day 24) estimated 6-10 weeks of focused engineering. Day 28 audit revised to ~4-6 weeks. Days 29-38 closures (III.A + III.B-staff + III.D + III.H both scopes + I.A + I.B + I.C + III.J + II.G Reassign + Day 36's II.B/E/F/G non-Reassign surface + Day 37's V.A BR4 + Day 38's deck-state cluster: SOD initial-active + re-touch + late-Departures reshuffle + forward-unlock) trim further. Practical path remains: chase the 8 items above back-to-back across ~1.5-2 focused weeks (queue dropped from 9 to 8 when chase #1 closed Day 38), then Vercel deploy + smoke test, then post-beta items (II.J KB Editor, II.K Calendar, II.L Recap, VII.H KB versioning, VIII.F-G ops) push off the critical path. Section II.J + II.K are post-beta per the spreadsheet's own marking — formal `[DEFER]` to a v2 lane satisfies "no cuts" without violating the promise.
...
## Last session close (Day 38, 2026-05-07)

Chase #1 closed end-to-end across 4 commits and 3 files. The Day 37 reading of "deck never unlocks past SOD" was a misdiagnosis at the clock-in layer; the actual root cause was at the deck-state layer — `active` defaulted to `useState("sod")` on mount even when SOD was already in the auto-derived `done` set, leaving the head bucket stuck in a "completed but active" state with no obvious path forward.

**Diagnostic process.** Probe-1 (`7f80ee7`) committed an uncommitted Day 38 patch that was sitting in the working tree from a prior aborted session — `.select()` zero-row guards on `clockIn` / `clockOut` in `lib/clock-in.ts`. The patch's headline diagnosis (RLS silently denying staff-role UPDATE) didn't hold up: `staff_update_authenticated` policy is `using (true) with check (true)` for any authenticated user. RLS audit before commit. Treated the patch as diagnostic plumbing, not a fix. UI test confirmed clock-in works end-to-end (UPDATE lands on `staff.clocked_in_at`, trigger fires, `shift_start` row written to `inbound_events`, deck renders). Probe-2 (`0dc1fb4`) added a temporary visible debug strip on `/staff` exposing `active`, `done`, `tasks.length`, per-bucket counts. Running the strip + click sequence with Bryan pinned the actual bug instantly: `done = [sod]` on initial load (correct — both SOD tasks have status='done'), but `active = sod` (the hardcoded default). The probe also surfaced Bryan's product-rule expectations — non-destructive re-touch on completed cards, and time-based reshuffle of Departures past 11 AM weekday / 12 PM weekend.

**Bundle commit (`b362102`)** shipped three pieces in one diff. (a) Bug 1 — `loadTasks` walks the dynamic bucket order after computing `initialDone` and `setActive`'s to the first non-done key; deck no longer renders with a "completed but active" head bucket. (b) Re-touch on completed cards via `.bcard.is-done { pointer-events: auto; cursor: pointer; }`; clicks now reach the card so `handleCardClick` can fire. (c) Late-Departures reshuffle — new pure helpers `isPastReshuffleTime(now)` + `computeBucketOrder(now, departuresIncomplete)` re-derive a dynamic `bucketOrder` via `useMemo`, reused in three call sites: deck render, `handleActionClick` advancement loop, `loadTasks` initial-active advancement. Reshuffle reuses existing `dispatch-config.ts` constants (`PRE_STAYOVER_RESHUFFLE_AT` Section 5 R13–R15, `PROPERTY_TIMEZONE`, `WEEKEND_DAY_NUMBERS`) — no new config. Probe-2 debug strip + the `bucketData.allCount` field that fed it removed in this same commit.

**Follow-up commit (`7173062`)** addressed Bryan's mid-test product clarification: clicking a checkmarked card should focus it without un-completing (view ≠ re-open), and from a focused-but-checkmarked state the user should be able to advance forward to the next eligible bucket directly. Two changes. (d) `handleCardClick` is non-destructive — drops the `setDone(prev.delete(key))` call so done state is preserved; clicking a done card just `setActive`'s it. (e) `is-unlockable` class on bucket cards whose previous buckets in the dynamic order are all in `done`, with CSS `pointer-events: auto` enabling forward-navigation clicks. Trade-off: explicit "re-open" / "un-complete" semantics no longer accessible via UI; tabled as post-beta enhancement (long-press, or "Mark not done" from card detail).

**UI-verified Day 38** (Lizzie via magic-link, real auth session, all 5 staff clocked out fresh state at session start):
- Re-touch back to SOD after Departures activates → SOD focuses, checkmark stays ✓
- Forward to Departures via SOD action button (still-checkmark) ✓
- Forward to Departures via direct click on Departures card body (`is-unlockable`) ✓
- Advance to Stayovers after clicking Departures action button ✓
- Re-touch back to Departures from Stayovers ✓

**Reshuffle UI verification deferred** to post-11 AM Central runtime check (Bryan completes solo). Day 38 SQL block 1 reset Lizzie's 2 incomplete Departures to status='open' for a clean test state; SQL block 2 cleared all 5 staff `clocked_in_at` for a fresh Pre-Clock-In flow. After 11:00 AM Central refresh, the deck should render in reshuffled order: SOD → Stayovers → Arrivals → Departures → Dailys → EOD, with first-non-done active = Stayovers (since SOD's `initialDone` and Departures has incomplete tasks).

**Day 38 friction surfaced (logged but not chased today):**
- **Stale uncommitted Day 38 patch in `lib/clock-in.ts`** from a prior aborted Cowork session — discovered during initial verification, treated as diagnostic plumbing and committed as probe-1. Process gap: the user-side CC verification at session start showed the modified file in `git status` collapsed under "+3 lines" and wasn't surfaced clearly. Future sessions should expand `git status` output literally before declaring "working tree clean." Standing tabled.
- **Stray-Lizzie data hygiene** re-confirmed with concrete UUIDs: orphan Lizzie at `fc2c4280-2be4-4ef8-a1ea-3a0b3dfbe3bc` (role 'Ops', no surname) vs. Lizzie Larson at `8fb2f515-4df3-4835-b2e9-e01f2eff993d` (role 'Front of House'). Lizzie's `profiles.staff_id` points at the orphan; AddTaskModal's "LL" chip writes Lizzie Larson's distinct UUID. Real fix is a Jennifer-confirmed canonical-Lizzie SQL migration; not chased Day 38, still in Open Jennifer questions.
- **`/staff` page's `now`** is set once at mount via `useState(() => new Date())`. Late-Departures reshuffle won't fire mid-session without a page refresh. Tabled (Standing tabled).
- **No explicit re-open / un-complete UI** post-Day-38. Done state is one-way via the action button; `handleCardClick` is now non-destructive. Tabled (Standing tabled).

**Followups for next session:** pick from the (now 8-item) chase queue. Quick-close lean is chase #1 (III.B Phase 5 `maintenance_issues` source swap in activity feed, ~30 min) — replaces placeholder `notes WHERE note_type='Maintenance'` source in `lib/activity-feed.ts:12` with a real `maintenance_issues` query branch. If Bryan wants a more substantial slice, chase #3 (II.H Admin Maintenance live-data wiring, ~1-2 hours) is the next significant Section II surface to land. Reading paths for chase #1: Tier 1 (STATE.md + CLAUDE.md) plus Tier 2 "Section III cross-cutter" — `lib/activity-feed.ts`, `lib/maintenance.ts`, `docs/TASK_EVENTS_CONTRACT.md`.
