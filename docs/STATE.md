# Dispatch — Product State (current as of Day 37, 2026-05-06)
...
**Section II:** II.A (`AddTaskModal`) is ~95% built per Day 28 audit; confirmation pass pending (now chase #6 below). II.B / II.E / II.F / II.G non-Reassign surface CLOSED Day 36. II.H Admin Maintenance still PARTIAL (chase #4 below).
...
1. **Pre-Clock-In "Start your day" / SOD start-shift button broken** (size unknown — 30 min to 2-3 hours). Surfaced Day 37 during V.A BR4 UI verification: from the staff home Pre-Clock-In screen / SOD card, the start-shift button click produces no `clocked_in_at` flip → no `shift_start` event → bucket deck never unlocks past SOD. Beta-blocker for all staff-side UI verification. Diagnostic-light starting points: `app/staff/page.tsx` (Pre-Clock-In screen + the start-shift CTA handler), `app/staff/task/[id]/StartOfDayCard.tsx` (if the button lives on the SOD card), `lib/clock-in.ts` (clockIn helper). Likely scope band: 30 min to 1 hour if it's a one-line click-handler / missing useEffect dep / RLS denial on `staff.clocked_in_at` UPDATE; worst-case 2-3 hours if the trigger chain (button → `clocked_in_at` flip → `staff_clock_in_event_trigger` → `inbound_events` write) has multiple broken links. Workaround used Day 37: direct URL to `/staff/task/<task_id>` bypasses the home-gate (RLS still gates the task fetch — see stray-Lizzie note in Open Jennifer questions for the data alignment that's required).

2. **III.B Phase 5 — `maintenance_issues` source swap in activity feed** (~30 min). Replace placeholder `notes WHERE note_type='Maintenance'` source in `lib/activity-feed.ts:12` with a real `maintenance_issues` query branch. Day 34's `reassigned`-as-warn classification + the existing severity-boost ordering means High-severity maintenance items will surface above info entries via the same path once the source swap lands.

3. **III.E + V.G photo pipeline wiring** (1-2 hours). `uploadTaskFile` helper exists at `lib/task-events.ts:45`. Wire into `NoteComposeForm` + `MaintenanceComposeForm`: file input + call `uploadTaskFile` + pass `imageUrl` to `addNote` / `addMaintenanceIssue`. Storage RLS policies need verification (master plan VII.F PARTIAL).

4. **II.H Admin Maintenance live-data wiring** (~1-2 hours). Schema unblocked by Day 33's `maintenance_issues` table. Wire `app/admin/maintenance/[id]/page.tsx` to query `public.maintenance_issues` filtered by location / by type, plus per-issue card view. Replace `ORDER` mock const.

5. **IV.H Wed-occupancy Deep Clean trigger** (~1 hour). Constants exist in `dispatch-config.ts` Section 12. Unblocked by III.D's audit-event sink (Day 29). All four conditions: <5 departures + 40%+ occupancy in last 45 days + no deep clean in 45 days + ≤3 deep items completed in 45 days. Auto-elevates Standard → Deep on Wednesdays.

6. **II.A `AddTaskModal` confirmation pass** (~30 min). Verify the modal matches master plan II.A spec end-to-end. Possible bucket-model tweak — maintenance currently routes to `staff_home_bucket: "start_of_day"` since maintenance has no staff bucket; confirm or surface as own bucket.

7. **I.G remaining sub-items.** Last Stayover Status lookup (was blocking on V.A BR4 — unblocked Day 37; sources from `tasks` history, not `reservations`), checklist variants for Sheet Change weekly + * guest (pending Jennifer's KB authoring), status-driven auto-complete (DND/Desk OK/Guest OK pre-selection auto-completes + auto-archives), Sheet Change skip semantics. Bundled or split as makes sense.

8. **Item I — Vercel deploy** (~30 min). Bryan's parallel lane via `docs/deployment/vercel-checklist.md`. GitHub push → Vercel CLI → first deploy → env vars (incl. `AGENT_KILL=true` and `AGENT_DRY_RUN=true` for safety) → Supabase magic-link redirect URL config → smoke test in incognito.

9. **V.C Cloudbeds.** Bryan's separate thread, outside engineering critical path. Pending sales quote.
...
The master plan footer (Day 24) estimated 6-10 weeks of focused engineering. Day 28 audit revised to ~4-6 weeks. Days 29-37 closures (III.A + III.B-staff + III.D + III.H both scopes + I.A + I.B + I.C + III.J + II.G Reassign + Day 36's II.B/E/F/G non-Reassign surface + Day 37's V.A BR4) trim further. Practical path remains: chase the 9 items above back-to-back across ~2-3 focused weeks (the queue grew from 8 to 9 mid-Day-37 when SOD start-shift bug surfaced — net work-in-flight), then Vercel deploy + smoke test, then post-beta items (II.J KB Editor, II.K Calendar, II.L Recap, VII.H KB versioning, VIII.F-G ops) push off the critical path. Section II.J + II.K are post-beta per the spreadsheet's own marking — formal `[DEFER]` to a v2 lane satisfies "no cuts" without violating the promise.
...
## Last session close (Day 37, 2026-05-06)

Chase #1 closed end-to-end: V.A BR4 X-430 brief reservation fallback. Commit `1657144`. Build clean. 4 files changed, 218 insertions / 12 deletions. No schema changes; no new dependencies; no CSS module changes.

**`app/staff/task/[id]/page.tsx`** — two new state slots (`currentReservation`, `incomingReservation` — both `Reservation | null`) and two new fetchers added to the existing notes/maintenance/checklist `Promise.all`. The fetch only fires when `task.room_number` is set AND the card_type surfaces guest fields: housekeeping_turn → both current (outgoing) + next-incoming, arrival → next-incoming, stayover → current. Helpers `getCurrentReservationForRoom` / `getNextIncomingReservationForRoom` already existed in `lib/reservations.ts` (Day 22 BR1 pack). `.catch()` wraps degrade to `null` silently with `console.warn`; the brief stays at em-dash placeholders on failure, same as today. Single `currentReservation` slot is shared between D-430 outgoing and S-430 current (same DB row, different framing); single `incomingReservation` slot is shared between D-430 incoming and A-430 incoming. Fetch + thread is single-pass.

**`app/staff/task/[id]/DeparturesCard.tsx`** — new helpers `isAllNull(GuestRecord)` and `guestRecordFromReservation(Reservation, "outgoing" | "incoming")`. Two new optional props: `outgoingReservation`, `incomingReservation`. Component body now does `parseGuestRecord(...)` first, then if all fields are null AND a reservation prop is non-null, swaps in the derived record. `clean_type` stays null on fallback (no reservation-side source — per-turn cleaning tier is KB-driven). Outgoing maps party_size → "guests" string; incoming maps party_size → "party" string + guest_notes/special_requests → notes.

**`app/staff/task/[id]/ArrivalsCard.tsx`** — new helper `incomingGuestFromReservation(Reservation)`. New optional prop `incomingReservation`. Existing `parseIncomingGuest()` already returns `null` cleanly when context subkey is missing, so the fallback is a clean `parsedGuest ?? fromReservation(...)` expression. Maps arrival_time → checkin_time, nights → nights (already number), party_size → party_size, special_requests joined → special_requests fallback to guest_notes.

**`app/staff/task/[id]/StayoversCard.tsx`** — new helper `currentGuestFromReservation(Reservation)` plus `todayInPropertyTz` import from `lib/reservations`. New optional prop `currentReservation`. Same `parsedGuest ?? fromReservation(...)` pattern. `nights_remaining` computed from `departure_date - todayInPropertyTz()` parsed at noon-local to avoid DST/zone rounding, formatted as "N night(s) left".

**UI-verified Day 37**: D-430 for room 25 (`task fe4b33f1-4bd6-4091-9988-6a68ef66cbc0`, no `context.outgoing_guest`/`incoming_guest`) renders Outgoing column "2 guests / 3 nights / —" — derived from Jane Roberts's live reservation row (room 25, status='arrived', arrival_date=current_date-3, departure_date=current_date, party_size=2, generated nights=3). Incoming column stays "—" because the seed has no future reservation for room 25 (clean half-verification of outgoing fallback path; A-430 + S-430 paths unverified by eye but exercise identical code-paths through the same page.tsx fetch + per-card fromReservation helpers).

Net for Day 37: chase queue trimmed from 9 to 8 (V.A closed) then re-grown to 9 (SOD start-shift bug added as new chase #1). Three Section I sub-items unblocked (I.E + I.F live guest data wirings; I.G Last Stayover Status lookup separately unblocked but remains its own work item — that one sources from `tasks` history, not `reservations`). Closure ledger gains a single V.A BR4 entry. The X-430 cards now degrade-gracefully across three regimes: (1) context guest subkey populated → use it (existing behavior preserved); (2) context missing + matching reservation exists → derive from reservation; (3) context missing + no reservation → em-dash placeholders (also existing behavior).

**Day 37 friction surfaced (logged but not chased today):**
- **SOD start-shift button broken** — added as chase #1 above. Beta-blocker for staff-side UI verification.
- **Stray Lizzie row vs Lizzie Larson confusion** (already in Open Jennifer questions) re-surfaced concretely as a verification blocker. Lizzie's `profiles.staff_id` points at the orphan `fc2c4280...` row, but AddTaskModal's "LL" assignee chip writes Lizzie Larson's distinct UUID. RLS `can_read_task` then fails for Lizzie viewing tasks assigned via the modal. Workaround Day 37: `update public.tasks set staff_id='fc2c4280-2be4-4ef8-a1ea-3a0b3dfbe3bc' where id=...` plus `assignee_name='Lizzie'`. Real fix: either pick one canonical Lizzie staff row + delete the other and re-point all profile.staff_id + task.staff_id references, or merge the two rows. Hour-band: 30 min if Jennifer confirms which to keep + we run a one-shot SQL migration; longer if there's existing task history split across both UUIDs.
- **AddTaskModal does not enforce `context.outgoing_guest` / `incoming_guest` / `current_guest` on housekeeping/arrival/stayover** (intentional — those subkeys are reservation-derivable per V.A BR4) but also doesn't preview the BR4 fallback at task-creation time. Acceptable for beta; cosmetic improvement for v2.

Followups for next session: pick from the (now 9-item) chase queue. Default lean is chase #1 (SOD start-shift button debug — beta-blocker; size unknown but likely 30 min - 1 hour band). Alternative quick close if appetite is for a known-tight scope: chase #2 (III.B Phase 5 `maintenance_issues` source swap in activity feed, ~30 min). If picking #1 (SOD), tier-2 reading paths to load: Section I staff card chase entries from "Reading paths by chase type" — `app/staff/page.tsx`, `lib/clock-in.ts`, `lib/dispatch-config.ts` Sections 9 + 12.
