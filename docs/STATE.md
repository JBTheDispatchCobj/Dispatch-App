# Dispatch — Product State (current as of Day 42, 2026-05-07)
...
## Closure ledger — master plan items closed (Days 27-42)
...
**Section II:** II.A (`AddTaskModal`) is ~95% built per Day 28 audit; confirmation pass pending (now chase #2 below). II.B / II.E / II.F / II.G non-Reassign surface CLOSED Day 36. II.H non-master-table surface CLOSED Day 41; II.H Phase C master-table index CLOSED Day 42. **Section II 100% closed end-to-end** (modulo II.A confirmation pass — process pass, not net-new build). Post-beta items II.J KB Editor + II.K Calendar + II.L Recap stay explicitly deferred to v2 lane per master plan footer.
...
1. **IV.H Wed-occupancy Deep Clean trigger** (~1 hour). Constants exist in `dispatch-config.ts` Section 12. Unblocked by III.D's audit-event sink (Day 29). All four conditions: <5 departures + 40%+ occupancy in last 45 days + no deep clean in 45 days + ≤3 deep items completed in 45 days. Auto-elevates Standard → Deep on Wednesdays. Pure orchestrator-side rule file work; lighter UI surface than recent chases.

2. **II.A `AddTaskModal` confirmation pass** (~30 min). Verify the modal matches master plan II.A spec end-to-end. Possible bucket-model tweak — maintenance currently routes to `staff_home_bucket: "start_of_day"` since maintenance has no staff bucket; confirm or surface as own bucket.

3. **I.G remaining sub-items.** Last Stayover Status lookup (was blocking on V.A BR4 — unblocked Day 37; sources from `tasks` history, not `reservations`), checklist variants for Sheet Change weekly + * guest (pending Jennifer's KB authoring), status-driven auto-complete (DND/Desk OK/Guest OK pre-selection auto-completes + auto-archives), Sheet Change skip semantics. Bundled or split as makes sense.

4. **Item I — Vercel deploy** (~30 min). Bryan's parallel lane via `docs/deployment/vercel-checklist.md`. GitHub push → Vercel CLI → first deploy → env vars (incl. `AGENT_KILL=true` and `AGENT_DRY_RUN=true` for safety) → Supabase magic-link redirect URL config → smoke test in incognito.

5. **V.C Cloudbeds.** Bryan's separate thread, outside engineering critical path. Pending sales quote.
...
- `/admin/maintenance/[id]` back-arrow points to `/admin` (not `/admin/maintenance`). When admins enter the per-issue page from the new master-table page (Day 42 chase #1), tapping back returns them to the home, not the list view they came from. Single-line change to the navBtn `href` in `[id]/page.tsx` if Bryan wants context-aware return-to-list behavior — trade-off is the back arrow becomes context-dependent (which list view did the user come from? — Watchlist on /admin, master-table on /admin/maintenance, or direct URL?), versus the current always-`/admin` simplicity. Day 42 Bryan-confirmed OK as-is during the 5-step UI verification.
...
## Last session close (Day 42, 2026-05-07)

Chase #1 closed in two commits — II.H Phase C master-table index at `/admin/maintenance` landed at `6f11c83`, then a small inbound-nav follow-up re-targeting the Maintenance lane tile on /admin home landed at `a20b09b` after Bryan's UI verification step 2 surfaced the stale link. Net work: ~50 min including diagnosis, plan-before-cut review, code cuts, build verify, two-commit close, and end-to-end 5-step UI verification with Bryan. **Section II now 100% closed end-to-end.**

**Diagnosis.** Master plan §II.H second leg: "master tables: open issues by location, open issues by type." Two interpretations possible — (a) two separate per-dimension pages or (b) one page with both dimensions filterable. STATE.md's Day 41 chase queue had pinned (b): "Filter UI: two flat dropdowns (Location selector + Type selector ...) with an 'All' option for unfiltered view." Smaller surface, closes the spec end with one new file pair. Confirmed at plan-before-cut.

**Inbound nav gap (compounding).** Day 41 closed the per-issue inbound nav gap (no link from /admin to /admin/maintenance/[id]); Day 42 surfaced and closed the master-table inbound nav gap one layer up (no link from /admin to /admin/maintenance, plus a stale Link from /admin's Maintenance lane tile to /admin/tasks#maintenance — the empty `card_type='maintenance'` lane partition on the tasks dashboard, a tasks-side concept distinct from `maintenance_issues` rows). Both are one-layer-up versions of the same problem. Forward check for next session: when adding a new admin surface, audit existing /admin home tiles + lane heads for stale routes BEFORE the chase closes — saves a follow-up commit.

**Implementation (`6f11c83`).** 3 files changed, 678 insertions / 2 deletions. (1) New file `app/admin/maintenance/page.tsx` — admin-gated client component cloning the [id]/page.tsx auth pattern. State: `locationFilter` + `typeFilter` (both default to "All" sentinel), `rows`, `loaded`, `error`. Single Supabase query against `public.maintenance_issues` with `.is("resolved_at", null)` always (master-plan spec is "open issues"); conditionally chains `.eq("location", X)` and/or `.eq("type", Y)` when filter !== "All"; `.order("created_at", { ascending: false }).limit(100)`. Severity desc client-side (High → Normal → Low) mirroring the Watchlist convention at `app/admin/page.tsx:82-83`. Sage hero strip with `OPEN ISSUES {count}` + `BY LOCATION · BY TYPE` axis label. Filter panel with two `<select>` dropdowns sourced from `MAINTENANCE_LOCATIONS` + `MAINTENANCE_TYPES` static exports in `lib/maintenance.ts`; conditional CLEAR button in panel head when either filter is set. Row list: `${location} — ${item}` title, mono meta line `${type} · RM ${room} · ${MMM DD}`, body excerpt (70-char truncation, mirrors Watchlist `excerpt` helper), severity chip variants `.rowChipHigh` (red `#A8423E`) / `.rowChipNormal` (sage) / `.rowChipLow` (white-with-muted-border). Each row wrapped in `<Link href={\`/admin/maintenance/${row.id}\`}>` (per-issue inbound nav already wired Day 41). Empty-state copy distinguishes filtered ("No open issues match this filter.") from unfiltered ("No open maintenance issues."). Cast pattern: `data as unknown as MaintIssueLite[]` per the Day 41 [id]/page.tsx:179 + `lib/maintenance.ts:138` precedent (supabase-js GenericStringError). (2) New file `app/admin/maintenance/page.module.css` — reuses sage tokens (`--sage-body`, `--sage-header`, `--sage-text`) + cream shell + grain noise SVG pattern from `[id]/page.module.css` so the two surfaces feel like one product. New blocks: `.filters` two-column grid, `.filterLabel` + `.filterLabelText` + `.filterSelect` (custom dropdown chevron via inline SVG), `.rowList` + `.rowLink` (block, no decoration, color inherit), `.row` (1fr auto grid for content + chip), `.rowChip{High,Normal,Low}` severity chip variants, `.empty` centered mono message, `.clearBtn` panel-head clear-filter button. (3) `app/admin/page.tsx` — Watchlist lane's conditional `{watchlistItems.length > 5 && <div>+{N} more</div>}` static caption swapped to a conditional `{watchlistItems.length > 0 && <Link href="/admin/maintenance" className={styles.laneItemMore}>VIEW ALL OPEN ISSUES | VIEW ALL · +{N} MORE</Link>}` (3 → 9 lines). Suppressed when watchlist is empty. Build clean on first pass; no TypeScript-strict trips because the cast widening was already in place from drafting.

**Maintenance tile follow-up (`a20b09b`).** Bryan's UI verification at step 2 surfaced a stale link: the Maintenance lane tile on /admin home (the diamond-icon card showing `{N} open` sub-line) was routing to `/admin/tasks#maintenance`, the empty `card_type='maintenance'` lane partition on the tasks dashboard. Re-targets the `Link href` to `/admin/maintenance` so the tile's open-count affordance + click destination are consistent. The open count it shows already comes from `watchlistItems` (the same fetch that powers the new master-table data), so no other change needed. 1 file changed, 1 insertion / 1 deletion. Build clean. Lands on top of `6f11c83`.

**UI verification ran clean Day 42** (unlike Day 41's [id] page UI verification, which is still pending Bryan-solo — multiple-day carry-forward). All five steps passed end-to-end:
1. Open `/admin`, expand Watchlist lane. Tap "VIEW ALL OPEN ISSUES" / "VIEW ALL · +N MORE" caption Link → routes to `/admin/maintenance`.
2. Tap Maintenance lane tile (diamond icon, `{N} open` sub-line) on `/admin` → after `a20b09b` re-route, lands on `/admin/maintenance` (same destination as the Watchlist VIEW ALL link).
3. Hero strip shows `OPEN ISSUES {count}` + `BY LOCATION · BY TYPE`. Row list displays severity-ordered (red High → sage Normal → white Low), with location — item / mono meta / body excerpt per row.
4. Pick a Location from the dropdown → list filters in place, count updates in panel head + hero strip, CLEAR button appears in FILTERS panel head. Pick a Type → filters compose. Tap CLEAR → both reset to "All", full backlog returns.
5. Tap any row → navigates to `/admin/maintenance/{id}` (the Day 41 per-issue page). Tap back arrow → returns to `/admin` (not `/admin/maintenance` — current behavior; logged in Standing tabled as a context-aware return-to-list trade-off).

**Build clean both passes.** Both commits ran build verify cleanly. CC used the alternative `npm run build 2>&1; echo "EXIT:$?"` fail-loud pattern on the first commit's build (build-then-commit-and-push as separate calls) and the chained-`&&` pattern on the second; both EXIT codes were 0; both pushes landed. End state matched the chained-pattern outcome. 22 routes total post-Phase-C; `/admin/maintenance` shows as static.

**Day 42 process notes:**
- **+N line collapses** continued the recurring pattern. `+5 lines` on git status pre-commit (substantive — expanded), `+47 lines` on the build readout (not expanded, but `EXIT:0` marker confirmed clean), `+6 lines` post-push (non-load-bearing tail — git log + push URL). Same triage as Day 41.
- **Inbound nav gap pattern compounding.** Day 41 closed the per-issue inbound nav gap; Day 42 closed the master-table inbound nav gap one layer up. Both were the same problem (no entry point into a new admin surface). Forward check codified above.
- **Mid-session UI surface fix worked clean.** Bryan's UI verification at step 2 caught the stale Maintenance tile route; surfaced as a small follow-up commit `a20b09b` rather than rolling into the main chase commit (preserves single-purpose-per-commit hygiene). Re-test for step 2 alone after the second commit, then steps 3-5 ran clean in sequence.
- **Plan-before-cut convention worked clean.** Diagnosis surfaced both inbound-nav gaps before code touched (Watchlist `+N more` → Link swap explicitly bundled in plan; Maintenance tile re-route surfaced during UI verification step 2 and closed as the follow-up). Code cuts hit the planned shape on first pass; build cleared cleanly twice.

**Carry-forward still pending Bryan-solo:**
- **Day 41 II.H per-issue page UI verification** — 4-step path (severity chip writeback to `maintenance_issues.severity`, Mark Resolved write-path stamping `resolved_at` + `resolved_by_user_id`, photo panel render when `image_url` is set, status dot blue → green flip, activity panel gains a "Admin marked resolved" row). Phase C verification today opened the per-issue page (step 5) but didn't exercise severity-chip writeback or Mark Resolved. Multiple-day carry-forward.
- **Day 40 III.E + V.G UI verification** — 4-step photo pipeline path (camera/library sheet, photo thumbnail on note + maintenance rows, image_attached event row in /admin/tasks/[id] activity panel, Storage file under `<user-uid>/` path). Multiple-day carry-forward.
- **Day 38 reshuffle UI verification** post-11 AM Central runtime check. After 11:00 AM Central refresh, the deck should render in reshuffled order: SOD → Stayovers → Arrivals → Departures → Dailys → EOD, with first-non-done active = Stayovers. Multiple-day carry-forward.
- **Stray-Lizzie data hygiene** unresolved (orphan `fc2c4280-2be4-4ef8-a1ea-3a0b3dfbe3bc` 'Ops' vs. canonical Lizzie Larson `8fb2f515-4df3-4835-b2e9-e01f2eff993d` 'Front of House'). Open Jennifer question; not chased Day 42.

**Followups for next session:** chase queue drops to 5 (II.H Phase C closed; Section II 100%). Highest-priority remaining is the new chase #1 — IV.H Wed-occupancy Deep Clean trigger, ~1 hour. Pure orchestrator-side rule file work; lighter UI surface than recent chases. Constants exist in `dispatch-config.ts` Section 12. Unblocked by III.D's audit-event sink (Day 29). All four conditions: <5 departures + 40%+ occupancy in last 45 days + no deep clean in 45 days + ≤3 deep items completed in 45 days. Auto-elevates Standard → Deep on Wednesdays. Reading paths: Tier 1 (STATE.md + CLAUDE.md) plus Tier 2 "Section IV rule engine chase" — `lib/orchestration/{run,interpret,assignment-policies,reshuffle,audit-events}.ts`, the relevant rule file under `lib/orchestration/rules/` (confirm path on read — likely `housekeeping.ts` or a dedicated `deep-clean.ts`), `lib/dispatch-config.ts` Section 12 for constants, `docs/TASK_EVENTS_CONTRACT.md` if adding new audit event types. Note Day 29 caveat: orchestrator-side modules use `.ts` extensions and inline string literals to be Node-safe; anything under `lib/orchestration/` that imports a browser-coupled module (one that imports `lib/supabase`) compiles but fails at orchestrator runtime. `lib/orchestration/index.ts` is the deliberate exception (browser-side per-task lifecycle boundary).
