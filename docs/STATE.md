# Dispatch — Product State (current as of Day 40, 2026-05-07)
...
- `/admin/maintenance/[id]` (mostly mocked — depends on II.H wiring; carry-forward chase #1).
...
## Closure ledger — master plan items closed (Days 27-40)
...
**Section II:** II.A (`AddTaskModal`) is ~95% built per Day 28 audit; confirmation pass pending (now chase #3 below). II.B / II.E / II.F / II.G non-Reassign surface CLOSED Day 36. II.H Admin Maintenance still PARTIAL (chase #1 below).
...
1. **II.H Admin Maintenance live-data wiring** (~1-2 hours). Schema unblocked by Day 33's `maintenance_issues` table. Wire `app/admin/maintenance/[id]/page.tsx` to query `public.maintenance_issues` filtered by location / by type, plus per-issue card view. Replace `ORDER` mock const. Closes the last partial in Section II.
...
The master plan footer (Day 24) estimated 6-10 weeks of focused engineering. Day 28 audit revised to ~4-6 weeks. Days 29-40 closures (III.A + III.B all five phases + III.D + III.E + III.H both scopes + I.A + I.B + I.C + III.J + II.G Reassign + Day 36's II.B/E/F/G non-Reassign surface + Day 37's V.A BR4 + Day 38's deck-state cluster: SOD initial-active + re-touch + late-Departures reshuffle + forward-unlock + Day 39's III.B Phase 5 maintenance source swap + Day 40's III.E + V.G photo pipeline + VII.F audit pass) trim further. Practical path remains: chase the 6 items above back-to-back across ~1-1.5 focused weeks (queue dropped from 9 to 6 across Days 38-40 — chase #1 SOD/deck-advancement Day 38, chase #1 III.B Phase 5 Day 39, chase #1 III.E + V.G Day 40), then Vercel deploy + smoke test, then post-beta items (II.J KB Editor, II.K Calendar, II.L Recap, VII.H KB versioning, VIII.F-G ops) push off the critical path. Section II.J + II.K are post-beta per the spreadsheet's own marking — formal `[DEFER]` to a v2 lane satisfies "no cuts" without violating the promise.
...
## Last session close (Day 40, 2026-05-07)

Chase #1 closed in a single commit (preceded by `dfb2e38` — markdown-only Day 39 STATE.md UI-verification line at session start). III.E + V.G — photo upload pipeline wired into both staff compose drawers, plus VII.F audit pass — landed as `84ad9c7`. Net work: ~70 min including diagnosis, plan, all-cards thread, and read-side image render. The chase as STATE.md scoped it (upload-only, ~1-2 hours) expanded to also include read-side render after the diagnosis exposed a half-feature risk. Bryan's call (Option A): include read-side, ship a complete loop in the same commit.

**Diagnosis.** Upload helper `uploadTaskFile` already existed in `lib/task-events.ts:52` (STATE.md previously listed line 45 — ~7 lines of drift, fixed in this STATE.md update). Path convention `${userId}/${timestamp}-${rand}.${ext}` matches the `task_files_insert_own` policy contract in `cards_mvp.sql:377-382` exactly (first folder = `auth.uid()::text`). Bucket `task-files` is public so `getPublicUrl` returns a URL anyone authenticated reads via `task_files_read`. Helper was complete; nothing to change there. Both compose forms had explicit "// What's NOT here: Image attachment input" comments dating from Day 24 / Day 33. State for the existing dropdowns + body was already threaded from `page.tsx` through every X-430 card via props (parent owns state, cards are pure presentational pass-throughs) — adding photo state followed the same shape: one new prop pair (`file: File | null`, `setFile: (f: File | null) => void`) per compose form, threaded through every card type's prop type, destructure, and compose mount.

**Read-side gap exposed.** Grep on `image_url` across `app/`: only the legacy `app/tasks/[id]/manager-card-detail.tsx` rendered photos, and that reads from the deprecated `task_comments` table (STATE.md confirms staff side stopped writing there post-Day-27). The new `notes` and `maintenance_issues` tables both carry `image_url` columns flowing through `NoteRow` + `MaintenanceIssueRow`, but no consumer rendered it. So upload-only would have shipped invisible artifacts. Read-side `<img>` block added to all 11 render surfaces: 5 X-430 cards mount note rows (Arrivals, Stayovers, Dailys, StartOfDay, EOD — Departures has count-only, no body render), 5 X-430 cards mount maintenance rows (Departures, Arrivals, Stayovers, Dailys, StartOfDay — EOD has no Maintenance host per master plan I.I), plus the legacy `page.tsx` fallback hosts both. Each render is a 14-line conditional block: `<a target="_blank" rel="noreferrer">` wrapping an `<img className="staff-photo-thumb">`. Inside the `<button>`-wrapped note rows in 5 cards an `onClick={(e) => e.stopPropagation()}` is attached to the `<a>` so tapping the photo doesn't fire the surrounding button click; HTML semantics warning (interactive content nested) is accepted as pragmatic — refactoring `<button>` to `<div>` would be aesthetics-only per CLAUDE.md "Do not refactor working code for aesthetics."

**Implementation (`84ad9c7`).** 10 files changed, 536 insertions / 9 deletions. (1) `NoteComposeForm.tsx` + (2) `MaintenanceComposeForm.tsx` — added `file` / `setFile` props; rendered the styled file-input button below the textarea, above the submit. iOS Safari `capture="environment"` hint surfaces the camera/library sheet directly. (3) `page.tsx` — added `noteFile` / `maintFile` state slots; imported `uploadTaskFile` + `logTaskEvent` + `taskEventType` + `withTaskEventSchema` from `lib/task-events`. Wired upload BEFORE row insert in `onPostNote` + `onPostMaintenance` with fail-loud on null return ("Photo upload failed. Try again, or remove the photo and post the note/issue without it."). On successful insert AND non-null `imageUrl`, fired `taskEventType.imageAttached` with `withTaskEventSchema({ image_url, source: "note" | "maintenance" })` — first call site for `imageAttached` in v1 vocabulary; surfaces in admin per-task event panel on `/admin/tasks/[id]`. Cleared file state on success alongside body (sticky-filters pattern preserved). Threaded new state down to all 6 X-430 card mounts + the two legacy fallback compose form mounts. Added 14-line image-render conditional to both legacy fallback row blocks. (4-9) `{Departures,Arrivals,Stayovers,Dailys,StartOfDay,EOD}Card.tsx` — added new prop pair(s) to prop type, destructure, compose mount. EOD only got the Notes pair (no Maintenance host). Five cards added image-render conditional to note rows; five cards added it to maintenance rows. (10) `globals.css` — new ~50 lines covering `.staff-attach-row` (flex container, gap 8px, wrap), `.staff-attach-btn` (label as button: 8/14 padding, gray-100 background, border, 6px radius), `.staff-attach-input` (1×1px hidden, opacity 0, no pointer events), `.staff-attach-name` (filename ellipsis, max-width 160), `.staff-attach-clear` (× button), plus `.staff-photo-thumb-link` (display block, 8px top margin) + `.staff-photo-thumb` (max-width/max-height 200px, 6px radius, border, object-fit cover).

**VII.F audit decision logged.** Two storage policies in `cards_mvp.sql:373-382`. `task_files_read` is permissive — any authenticated user reads any object in the bucket. `task_files_insert_own` is restrictive — caller's `auth.uid()::text` must equal the first folder, so each user writes only into their own UID prefix. Bucket is public so public-URL render works without auth. For beta single-property scope this is appropriate — photos are operational artifacts (broken TV, stained linen), not sensitive PII. Master plan VII.F's tighten path scopes SELECT through `can_read_task` so non-author non-admin can't enumerate URLs — but that forces a switch from public-URL `<img src>` to signed-URL async fetch with token rotation, expiry, loading states. Meaningful client-side surgery for marginal beta benefit. Logged in Standing tabled with the trade-off documented; revisit post-beta when tenancy boundaries get real.

**Vocabulary decision.** `imageAttached` event type already existed in v1 vocabulary at `lib/task-events.ts:11` — first call site is now this Day 40 wiring; no schema bump needed. Detail payload follows the schema_version: 1 contract via `withTaskEventSchema`. `source` discriminator (`"note" | "maintenance"`) lets the future admin event-panel renderer distinguish (currently all `image_attached` rows render uniformly via the per-task event panel).

**Build clean.** `84ad9c7` on origin/main; working tree clean post-commit. Build verified pre-commit via the standard `&&`-chained CC prompt — no `tail` pipe per the standing convention. 10 files changed, 536+/9-, matches the touch list exactly.

**UI verification path** for Bryan post-deploy (not yet run — Day 40 closed in dev; UI test pending after next deploy or local run):
1. Open `/staff` → clock in → open any active card with Notes section → tap "Add photo" button → camera or library sheet opens → snap or pick → file name appears next to button → fill body + type → Post → photo appears as a thumbnail on the just-posted note row.
2. Same for Maintenance compose drawer (file is fully optional; can post taxonomy-only or photo-only).
3. Open `/admin/tasks/[id]` for that task → Activity panel shows the `image_attached` event row.
4. Open Supabase dashboard → Storage → `task-files` bucket → confirm the file lives under `<your-user-uid>/...` path (first-folder = uid contract).

**Day 40 process notes:**
- **Day 38/39 +N line collapse pattern surfaced twice.** First as `+3 lines` after `git add docs/STATE.md`; second as `+4 lines` post-push on the verification readout. In both cases the collapsed content was the standard staged-section / commit-log output — no untracked file slipped in. Pattern continues; standing tabled per Day 39 close. Cowork-Claude flagged each instance and asked Bryan to expand pre-acceptance.
- **STATE.md line drift caught.** `uploadTaskFile` was listed at `lib/task-events.ts:45` in three places in STATE.md; actual line is 52. Drift fixed inline as part of the Day 40 update. Chase-time grep on the actual symbol (rather than the listed line number) caught it without consequence.

**Carry-forward from Days 38-39 still pending Bryan-solo:**
...
- **Stray-Lizzie data hygiene** unresolved (orphan `fc2c4280-2be4-4ef8-a1ea-3a0b3dfbe3bc` 'Ops' vs. canonical Lizzie Larson `8fb2f515-4df3-4835-b2e9-e01f2eff993d` 'Front of House'). Open Jennifer question; not chased Day 40.

**Followups for next session:** chase queue dropped from 7 to 6. Highest-priority remaining is chase #1 (II.H Admin Maintenance live-data wiring, ~1-2 hours) — would close the last partial in Section II by wiring `app/admin/maintenance/[id]/page.tsx` to `public.maintenance_issues` filtered by location / by type, plus per-issue card view. Reading paths for chase #1: Tier 1 (STATE.md + CLAUDE.md) plus Tier 2 "Section II admin surfaces chase" — `app/admin/maintenance/[id]/page.tsx`, `lib/maintenance.ts` (already pattern-matched in lib/notes.ts via dual-sink + denormalize), `app/admin/staff/[id]/page.tsx` as the live-data reference pattern (already wired Day 32 to the views), `docs/supabase/maintenance_issues_table.sql` for schema reference. Substantial slice alternative is chase #2 (IV.H Wed-occupancy Deep Clean trigger, ~1 hour). Day 40's photo pipeline closure means ALL Section III items in the chase queue are now closed; remaining queue is II + IV + I.G + deploy + Cloudbeds.
