...
**UI-verified Day 39** (Bryan, post-session, all four steps pass): Kind dropdown gained Maintenance option (between Notes and the Refresh button) ✓; filter to Maintenance shows existing `maintenance_issues` rows with the MAINT badge in row meta ✓; severity-boost ordering places High-severity maintenance rows above Info entries (alongside `reassigned` events in the warn band) ✓; filter combinations (Severity=Warn + Kind=Maintenance returns the High-severity maintenance subset; Kind=Maintenance + Severity=Critical returns empty per current severity-mapping choice) behave as expected ✓.
