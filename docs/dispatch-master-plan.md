...
**Headline count: 3 items remaining** = 2 master plan items across Sections I–X + 0 STATE.md standing-tabled SHIP-NOW items + 1 STATE.md Open Jennifer question. *Day 52 chases #1-#6 closed 19 inventory items end-to-end (cumulative 22 → 3). Chase #6: V.C DROP (Cloudbeds dead — ResNexus is actual channel manager) + VII.F/H/I formal-DEFER + VI.F authoritatively closed (Jennifer's Rules for Housekeeping line 106 matches `lib/dispatch-config.ts` Section 9 EXACTLY for all 12 non-ADA cells; combined with Q5 ADA confirmation, all 18 cells now authoritatively confirmed; Q4 misunderstanding resolved — values were Jennifer's all along) + IV.H Phase B Bryan-decisions LOCKED (denominator = 21 from ResNexus CSV row count; data source = Adults/Children/Pets aggregate row) + 2 KB archives shipped (Rules for HouseKeeping.docx.md + channel-manager-data-shape.csv) + 1 small `lib/dispatch-config.ts` comment update (placeholders → CONFIRMED).* The 2 remaining master plan items: VI.E (Sheet Change/Pet/VIP/Long-term variant CHECKLISTS Jennifer-pending KB; thresholds + scope resolved Day 52 chase #6 via 7-night/14-day clarification from Rules for Housekeeping) + VI.G (per-task time estimates Jennifer-pending KB ETA "within the week"). The 1 remaining Open Jennifer question: Q21 maintenance cascade tree (Jennifer KB authoring). **Engineering work surface remaining: ZERO active.** All forward motion waits on Jennifer KB. Bryan-actionable engineering follow-ups (queued, not chase-queue): Chase E Deep Clean +1, IV.H Phase B single-line activation, ResNexus CSV import path.
...
### Section V — Data layer + integrations (0 active items, Cloudbeds DROPPED)

*V.C DROPPED Day 52 chase #6 — Cloudbeds is dead per Bryan. ResNexus is the actual channel manager and was always intended to be (CLAUDE.md "Future (post-beta): ResNexus channel-manager integration for real room/reservation data"). V.H pairs V.C — also DROPPED (was already in Day 51 chase #8 formal-DEFER subsection; reframe as DROP since ResNexus is the path, not Cloudbeds). New Bryan-actionable engineering chase: ResNexus CSV import path. Schema-implication reference archived at `docs/kb/channel-manager-data-shape.csv` (ResNexus export sample). Group-booking shape (same reservation ID across multiple rooms) is the non-trivial piece to design around.*

- *V.C DROPPED Day 52 chase #6 — Cloudbeds dead. ResNexus is the channel manager. Future ResNexus CSV import path is queued as new engineering chase territory; not chase-queue until Bryan picks up.*
...
### Section VI — Jennifer's KB authoring lane (2 items remaining + 1 closed Day 53, parallel non-blocking)

*Day 52 chase #1 closed 4 items (VI.A / VI.C / VI.D placeholders accepted; VI.H declined). Day 52 chase #3 closed VI.B (canonical-list replacement). Day 52 chase #6 closed VI.F (Jennifer's Rules for Housekeeping line 106 matches `dispatch-config.ts` Section 9 exactly — values were authoritative all along; Q4 misunderstanding resolved). Day 53 chase #1 added VI.I (Maintenance Dropdowns substrate landed) and partial-progressed VI.E (Deep Clean + Pet Clean substrate landed in `docs/kb/`; Pet how-to text + Sheet Change/\*\*\*/Long-term variant checklists still pending). Inventory drops VI 8 → 4 → 3 → 2 → still 2 active (VI.E + VI.G); +1 new VI.I closed same-chase. The two below remain active.*
...
- **VI.E — Variant CHECKLISTS** (Sheet Change, Pet, VIP, Long-term). Jennifer Q2 PARTIAL: Deep Clean +1 item (the parent line linking to existing Deep Clean section) **SHIPPED Day 53 chase #2 (Chase E)** — conditional trigger seed via `context.outgoing_guest.clean_type='Deep'` in `tasks_seed_default_checklist()` (`docs/supabase/checklist_seed_per_card_type_with_deep.sql`, supersedes Day 52 chase #3 SQL); UI canonical via `DEPARTURES_DEEP_CANONICAL_CHECKLIST` in `lib/staff-task-execution-checklist.ts`; `DeparturesCard.buildDisplayChecklist` branches on `outgoing.clean_type === 'Deep'`; backfill SQL for existing Deep tasks. VIP guests get prominent banner at top of A-430/S-430 (SHIPPED Day 52 chase #4 — `task.context.vip` flag + bucket-agnostic `.vip-banner` CSS). **Day 53 chase #1 partial-progress**: Deep Clean substrate authored at `docs/kb/Departure_DeepClean_variant.md` (7-item sub-tree with how-to text from Bryan's Deep Clean Admin List CSV); Pet Clean substrate authored at `docs/kb/Departure_PetClean_variant.md` (+2 item names from Day 52 chase #5 Rules Table xlsx D-430 row 30 mining; per-item how-to text pending Jennifer — Day 53 ask Q3). Sheet Change + \*\*\* + Long-term variant CHECKLISTS still pending Jennifer's KB (Day 53 ask Q2). Pet Clean orchestration wiring (analogous to Chase E for Deep) still pending Jennifer per-item how-to text. **Long-term scope RESOLVED Day 52 chase #6** via Rules for Housekeeping (`docs/kb/Rules for HouseKeeping.docx.md`): 7-night threshold for operational variants (Deep Clean elevation per line 97; Sheet Change scheduling per line 132; modified stayover time targets per line 118 — "for guests staying longer than 7 days should take 3-8 minutes"); 14-day threshold for "Longterm Prep" Setup notation per line 93. Long-term variant CHECKLIST itself still pending Jennifer's KB (only the threshold ambiguity is resolved).
...
*VI.I CLOSED Day 53 chase #1 — Maintenance Dropdowns substrate authored at `docs/kb/Maintenance_Dropdowns.md` from Bryan's upload of Jennifer's three lists (Location 12-parent / Item 11-parent / Type 10-flat). Closes the Q21 content gap surfaced by Day 52 chase #5 (rules-table xlsx didn't carry the lists). Doc includes reconciliation notes vs `lib/maintenance.ts` Day 33 ship (code's flat 21-locations matches operational coverage but lacks per-room enumeration sub-picker); code reconciliation chase deferred post-beta. Cascade-vs-flat behavior remains the narrowed open question — Day 53 ask Q4. See STATE.md Day 53 ledger for closure cite.*

*VI.F CLOSED Day 52 chase #6 — `lib/dispatch-config.ts` Section 9 `DEPARTURE_TIME_TARGET_MATRIX` values authoritatively confirmed against Jennifer's "Rules for Housekeeping" doc line 106 (archived at `docs/kb/Rules for HouseKeeping.docx.md`). All 12 non-ADA cells match Jennifer's verbatim text exactly: Standard Queen 30-45, Standard Double 35-50, Standard King Jacuzzi 45-60, Standard Suite 45-65, Deep Queen 60-120, Deep Double 70-130, Deep King Jacuzzi 75-150, Deep Suite 75-150, Pet Queen 60-120, Pet Double 70-130, Pet King Jacuzzi 75-150, Pet Suite 75-150 (all minutes). Combined with Q5 ADA confirmation (Day 52 chase #4 — ADA equal to non-ADA), all 18 cells confirmed. Q4 was a misunderstanding — Jennifer asked for a grid because she didn't realize her own Rules for Housekeeping doc already had the values; nobody connected the dots until Day 52 chase #6's archive pass. Comment block in `dispatch-config.ts` Section 9 updated to remove stale "placeholders pending her authoring pass" framing + cross-reference the archived doc.*
...
### Section VII — Schema + RLS (FORMAL DEFER closed, Day 52 chase #6)

*All 3 remaining items FORMALLY ACCEPTED Day 52 chase #6 as POST-BETA DEFERRED per Day 51 chase #7 precedent. Section VII reaches zero items remaining for active inventory tracking.*

- *VII.F DEFERRED — photo storage RLS tighten path. Audited Day 40, kept as-is for beta. Tighten requires switching from public-URL `<img src>` to signed-URL async fetch with token rotation + expiry + loading states; meaningful client-side surgery for marginal beta benefit (notes/maintenance photos are operational artifacts, not sensitive PII per Day 40 closure). Revisit post-beta when tenancy boundaries get real (multi-property or external-stakeholder access).*
- *VII.H DEFERRED — KB versioning + change history. Affects Updates panel highlighting + audit trail + rollback. Tied to II.J Admin KB Editing Tool (post-beta per master plan authoring intent). Revisit when II.J ships post-beta.*
- *VII.I DEFERRED — Weather + holiday calendar tables. Optional caching layer for V.D weather + V.E Google events APIs (both also post-beta per Day 51 chase #8). Live API calls fit beta scale fine. Revisit when V.D + V.E activate post-beta and per-call latency becomes operationally meaningful.*
...
### STATE.md Open Jennifer questions (3 items, refreshed end of Day 53 chase #1)

*Day 52 chases #1 + #4 + #5 + #6 closed 8 of the 9 prior questions (ADA cells confirmed Q5; D-430 tolerance confirmed strict per Q6 mining; AddTaskModal bucket flipped to dailys per Q20; reference Wednesday `2026-01-07` confirmed via Q24 payroll-cadence walkback; "Courtney Manager" + Stray Lizzie N/A per Q22+Q23 test-user data; Section 14 [ASK JENNIFER] flags promoted via Q25 7-role schema; IV.H Phase B Bryan-decisions LOCKED Day 52 chase #6 — denominator = 21 from ResNexus CSV row count, data source = Adults/Children/Pets aggregate row from CSV import). Day 53 chase #1 narrowed Q21 (lists landed via VI.I) and added 2 narrower questions from the Day 53 Jennifer batch ask. Current state:*

- Maintenance compose drawer — **flat-vs-cascade behavior** (lists landed Day 53 VI.I; cascade-vs-flat picker shape still pending). Day 53 ask Q4.
- **Pet Clean per-item how-to text** (Bedding + Floors items confirmed via Day 52 chase #5 mining; per-item how-to descriptions pending). Day 53 ask Q3.
- **Three variant Stayover checklists** — Sheet Change / \*\*\* (modified discounted) / Long-term Maintenance Stayover. Standard Stayover sections authored Day 52 chase #3; the three variants still need their own per-section authoring. Day 53 ask Q2.
