...
// task_events + notes, joins display names from profiles, normalizes to a
// unified ActivityFeedItem shape, and returns reverse-chronological with
// severity boost (criticals top → warns → info, then by created_at desc).
//
// Sources (per master plan III.D):
// - task_events (existing per-task append-only sink — 13 event_types
//   including the 3 audit events added in Phase 1).
// - notes (Day 27 III.A — staff-authored notes with 11/5/5 taxonomies).
// - Future: maintenance_issues (master plan VII.B PARTIAL — currently
//   sourced via notes where note_type='Maintenance' until that table lands).
...
//
// Query strategy: two parallel fetches via Promise.all, merge in TS, sort,
...
export type ActivityKind = "task_event" | "note";
...
/**
 * Load the property-wide activity feed. Reads task_events + notes in
 * parallel, normalizes, merges, sorts by severity-boosted reverse-chron,
 * applies filters, slices to limit. Default limit 100.
 *
 * Errors from either source log a warning and the source contributes zero
...
export async function getActivityFeed(
  ...
  const [taskEventItems, noteItems] = await Promise.all([
    fetchTaskEventItems(client, { limit: perSourceLimit }),
    fetchNoteItems(client, { limit: perSourceLimit }),
  ]);

  return mergeAndRank([...taskEventItems, ...noteItems], options, limit);
}

/**
 * Per-staff variant for II.E Admin Staff Profile (Phase 4 helper). Filters
 * task_events to user_id = userId AND notes to author_user_id = userId.
 * Same merge / sort / limit semantics as getActivityFeed.
 *
