...
//
// Current scope (cumulative through Step 5):
 ...
 *
 * Distribution (cumulative through Step 5):
 ...
 *   4. Track per-member load + starting hall + hall demand to drive the
 *      lighter-load tiebreak and the adjacency rule.
 *
...
export function assignDrafts(
  ...
  // PolicyState bundles per-batch tracking: load count, starting hall,
  // and remaining hall demand. Step 6 (no-orphan) will extend loads to
  // per-type-per-member to evaluate against STANDARD_LOAD_PER_HOUSEKEEPER.
  const state: PolicyState = {
    loads: new Map(),
    startingHalls: new Map(),
    hallDemand: new Map(),
  };
  for (const m of ctx.roster) state.loads.set(m.staff_id, 0);
  ...
  return sorted.map((draft) => {
    ...
    // Update state: load count, starting hall (first time only), and
    // decrement remaining hall demand for the assigned room's hall.
    state.loads.set(member.staff_id, (state.loads.get(member.staff_id) ?? 0) + 1);
...
type PolicyState = {
  /** staff_id → drafts assigned so far this batch. */
...
function pickLighterFromUnfiltered(
  ...
  if (pool.length === 0) return null;
  let best = pool[0];
  let bestLoad = state.loads.get(best.staff_id) ?? 0;
  for (let i = 1; i < pool.length; i++) {
    const m = pool[i];
    const l = state.loads.get(m.staff_id) ?? 0;
    if (l < bestLoad) {
      best = m;
      bestLoad = l;
    }
