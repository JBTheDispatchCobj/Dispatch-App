...
/**
 * Return shape for assignDrafts. The drafts array is the assigned (post-
 * sort) batch ready for bulk-insert. The pendingAudits array is aligned by
 * index — pendingAudits[i] is the list of structured audit signals fired
 * during the pick for drafts[i] (e.g., cross-hall override, above-standard
 * load). Empty array means no audits fired for that draft.
 *
 * Audit emission is deferred to run.ts because the audits need real
 * task_ids from the post-insert rows. See lib/orchestration/audit-events.ts
 * for the emission helper.
 */
...
/**
 * Assign every draft in the batch to a roster member. Returns the assigned
 * drafts plus a per-draft pendingAudits array (aligned by index). Pure —
 * does not mutate the input array or its members.
 *
 * Distribution (cumulative through Step 6 + Day 29 III.D Phase 1):
 ...
 *   5. Day 29 III.D Phase 1: when cross-hall override or above-standard
 *      load fires, push a PendingAudit to state.pendingAudits[i] (aligned
 *      with sorted-draft index). run.ts emits these as task_events rows
 *      after the bulk-insert with the freshly-generated task_ids.
 *
...
export function assignDrafts(
  ...
  if (ctx.roster.length === 0) {
    return { drafts: drafts.slice(), pendingAudits: drafts.map(() => []) };
  }
  ...
  // hall, remaining hall demand, and (Day 29) per-draft audit signals
  // collected for post-insert emission.
  const state: PolicyState = {
    loads: new Map(),
    startingHalls: new Map(),
    hallDemand: new Map(),
    pendingAudits: sorted.map(() => []),
    activeAuditIndex: 0,
  };
  ...
  const assignedDrafts = sorted.map((draft, idx) => {
    // Set the active audit index BEFORE any pick logic runs so any helper
    // that pushes to state.pendingAudits lands in the correct slot. Applies
    // to both pre-assigned and lane-assigned drafts (above-standard load
    // can fire from incrementLoadAndWarn for either path).
    state.activeAuditIndex = idx;
    ...
    // Update state: per-type load count (with above-standard audit),
  ...
  return { drafts: assignedDrafts, pendingAudits: state.pendingAudits };
}
...
type PolicyState = {
  ...
  /**
   * Per-draft audit signals collected during the assignment pass (Day 29
   * III.D Phase 1). Aligned by index with the post-sort drafts array. Each
   * slot is the list of PendingAudit signals fired for that draft (zero,
   * one, or both of cross_hall_override + above_standard_load).
   */
  pendingAudits: PendingAudit[][];
  /**
   * Index into pendingAudits being filled by the current map iteration.
   * Set at the top of each iteration in assignDrafts so helpers can push
   * without threading the index through every signature.
   */
...
function pickByLighterLoad(
  ...
  if (eligible.length === 0) {
    ...
    // whose starting hall still has unassigned drafts. Relax + warn + emit
    // structured audit (Day 29 III.D Phase 1). Console.warn stays for
    // dev-time narration of orchestrator runs; the audit event is what
    // surfaces in the III.D activity feed for admin review.
    ...
    if (best) {
      ...
      // Push audit signal aligned with the current draft. run.ts pairs
...
function incrementLoadAndWarn(
  ...
  if (counts[key] > threshold) {
    // Console.warn stays for dev-time narration; audit event is what
    // surfaces in the III.D activity feed. Day 29 III.D Phase 1.
    console.warn(
